...
function tagsPlots(datasets::Array{String,1},mesh::Float64,iter::Int64,step::Int64)
        ...
        for (i, dataset) in enumerate(datasets[2:end])
                ...
                plot!([mesh*j for j in 0:iter],average_clusters,linewidth=2,grid=false,linestyle=linestyles[i+1], framestyle=:box,label=labels[i+1], title="", xlabel="β", ylabel="# exchanges",legend=:outerright,legendfont=10)
        end
        savefig("../plots/exchanges.pdf")
        savefig("../plots/exchanges.png")
        PyPlot.clf()
        dataset=datasets[1]
        results=readdlm("../text/results$dataset.txt",Float64)
